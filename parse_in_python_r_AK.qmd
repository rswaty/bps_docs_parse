...
```{r}
reticulate::py_install("python-docx", pip = TRUE)
...
write.csv(modelers, file = 'tables_AK/modelers.csv', row.names = FALSE)
...
write.csv(indicator_species, file = 'tables_AK/bps_indicators.csv', row.names = FALSE)
```
...
fire_frequency <- bind_rows(all_tables) %>%
  select(BPS_MODEL, everything()) |>
  select(c(BPS_MODEL, Severity, Avg.FI, Percent.of.All.Fires, Min.FI, Max.FI  )) |>
  rename(bps_model_id = BPS_MODEL) |>
  clean_names()
  

#*** kb changed output location
write.csv(fire_frequency , file = 'tables_AK/fire_frequency.csv', row.names = FALSE)
```
